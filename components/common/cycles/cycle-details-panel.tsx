...
export function CycleDetailsPanel({ cycle, issues }: CycleDetailsPanelProps) {
   ...
   useEffect(() => setNameDraft(cycle.name), [cycle.name]);
         ...
         <div className="px-4 pt-4 shrink-0">
               ...
               <div className="flex items-center gap-1.5 flex-wrap">
                  <DropdownMenu>
                     <DropdownMenuTrigger
                        ...
                        className="text-xs px-2 py-1 rounded-md bg-accent text-muted-foreground outline-none hover:text-foreground"
                     >
            ...
            <div className="flex items-center gap-2 mt-4">
               ...
               <input
                  ...
                  className="text-lg font-semibold bg-transparent outline-none rounded-sm px-1 -mx-1 min-w-0 flex-1 hover:bg-accent/40 focus:bg-accent/40 transition-colors"
               />
               <button
                  aria-label={`Delete ${cycle.name}`}
                  title={
                     issuesInCycle > 0
                        ? `${issuesInCycle} issues are still in this cycle`
                        : 'Delete cycle'
                  }
                  disabled={issuesInCycle > 0}
                  onClick={async () => {
                     if (await remove(cycle.id, cycle.name)) closePanel();
                  }}
