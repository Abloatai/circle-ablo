...
const sortIssues = (issues: HydratedIssue[], ordering: string): HydratedIssue[] => {
   switch (ordering) {
      case 'created':
         return [...issues].sort(
...
const groupByKey = (
   issues: HydratedIssue[],
   keyOf: (issue: HydratedIssue) => string
): Map<string, HydratedIssue[]> => {
   const map = new Map<string, HydratedIssue[]>();
...
export const GroupedIssuesView: FC<GroupedIssuesViewProps> = ({
   ...
   const groups = useMemo<GroupEntry[]>(() => {
      ...
      const buildGroups = (): {
         ...
         switch (grouping) {
            case 'assignee': {
               const keyOf = (issue: HydratedIssue) => issue.assignee?.id ?? 'no-assignee';
            ...
            case 'project': {
               const keyOf = (issue: HydratedIssue) => issue.project?.id ?? 'no-project';
               const totals = groupByKey(scopeIssues, keyOf);
               const visible = groupByKey(visibleIssues, keyOf);
               return [...totals.entries()]
                  .sort((a, b) => b[1].length - a[1].length)
