...
export default function IssuePreview({ notification, onMarkAsRead }: IssuePreviewProps) {
   ...
   // The real description, not a fixture. Hooks cannot sit behind the early
   // return below, so this runs with an empty identifier when nothing is
   // selected, which resolves to no issue and no blocks.
   const { description } = useIssueDetail(notification?.identifier ?? '');
   ...
   // Live issue (falls back to the notification snapshot).
   const issue = allIssues.find((candidate) => candidate.identifier === notification.identifier);
   const displayIssue = issue ?? notification;
   // The properties panel still reads links, PRs and milestone from the
   // fixtures — those are not modelled yet. The body above is live.
         ...
         <div className="flex items-center justify-between px-4 h-10 border-b border-border shrink-0">
            ...
            <div className="flex items-center gap-2 shrink-0">
               ...
               <Button variant="ghost" size="xs" asChild>
                  <Link href={`/${orgId ?? organizationSlug}/issue/${displayIssue.identifier}`}>
                     Open
         ...
         <div className="flex-1 min-h-0 flex overflow-hidden">
            <div className="flex-1 min-w-0 overflow-y-auto">
               <div className="pt-8 pb-6 px-6 w-full max-w-3xl mx-auto">
                  ...
                  {/* Real description */}
                  <div className="mt-6">
                     <ContentBlocks blocks={description} />
                  </div>
