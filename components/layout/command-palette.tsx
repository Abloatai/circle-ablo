...
export function CommandPalette() {
   ...
   const { members: users, organizationSlug } = useWorkspace();
   ...
   // Live rows and real writes. These used to come from a Zustand store seeded
   // from the fixtures, so every change the palette made was in-memory: it
   // looked applied, and was gone on reload.
   const issues = useIssues();
   const { setStatus, setPriority, setAssignee, setLabels, setProject, setCycle, setDueDate } =
      useIssueActions();
   const { openModal } = useCreateIssueStore();

   // The workspace slug is the first URL segment; the fallback is the viewer's
   ...
   /**
    * Due-date shortcuts, relative to today.
    *
    * These were four literal dates written when the template was built, so
    * "Today" set a due date in the past and drifted further every day.
    */
   const dueDates = useMemo(() => {
      ...
      return [
         ...
         ['End of this week', iso(endOfWeek(today, { weekStartsOn: 1 }))],
         ['In one week', iso(addDays(today, 7))],
      ] as const;
   }, []);
               ...
               <CommandList className="max-h-96">
                  ...
                  {route === 'root' && issue && (
                     <>
                        <CommandGroup heading="Issue">
                           ...
                           <CommandItem
                              onSelect={() => {
                                 void setAssignee(issue.id, null);
                     ...
                     <CommandGroup heading="Set priority…">
                        {priorities.map((candidate) => (
                           <CommandItem
                              key={candidate.id}
                              onSelect={() => {
                                 void setPriority(issue.id, candidate.id);
                  ...
                  {route === 'due-date' && issue && (
                     <CommandGroup heading="Set due date…">
                        {dueDates.map(([label, date]) => (
                           <CommandItem
                              key={label}
                              onSelect={() => {
                                 void setDueDate(issue.id, date);
