...
/**
 * The four values `project.health` and `project_update.health` both store.
 * `no-update` is what the composer's "comment" mode writes: a note on the
 * project that deliberately makes no claim about how it is going, so it
 * renders without a badge and does not move the project's health.
 */
export type ProjectUpdateHealth = 'no-update' | 'on-track' | 'at-risk' | 'off-track';

/** The three a person can pick when posting an update. */
export const postableHealth = ['on-track', 'at-risk', 'off-track'] as const;
   ...
   '1': {
      ...
      description: [
         ...
         { type: 'heading', text: '2. Solution' },
         {
            type: 'bullet-list',
            items: [
               'Extract `useDismissable`, `useFocusScope` and `useAnchorPosition` into `@circle/behaviors`.',
